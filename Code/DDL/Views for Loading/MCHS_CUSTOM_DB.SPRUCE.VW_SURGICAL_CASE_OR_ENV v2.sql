-- MCHS_CUSTOM_DB.SPRUCE.VW_SURGICAL_CASE_OR_ENV v2.sql
...
--                        SEVOFLURANE.END_DT_TM)        AS GAS_DURATION_MINUTES      -- Based on SEVOFLURANE
      ,TIMEDIFF(MINUTE, LAG(GAS_TS,1,GAS_TS) 
               OVER (PARTITION BY NCHS_ONLY_SURGICAL_CASE_ID 
               ORDER BY NCHS_ONLY_SURGICAL_CASE_ID, GAS_TS), 
               GAS_TS)                                 AS GAS_DURATION_MINUTES
      ,MAX(CASE
...
--      ,GAS_TS
--      ,LAG(GAS_TS,1,GAS_TS) OVER (PARTITION BY NCHS_ONLY_SURGICAL_CASE_ID ORDER BY NCHS_ONLY_SURGICAL_CASE_ID, GAS_TS) AS PREV_GAS_TS 
--      ,TIMEDIFF(MINUTE, PREV_GAS_TS, GAS_TS) AS GAS_DURATION_MINUTES
...
--      ,NCHS_ONLY_SURGICAL_CASE_ID -- Only NCHS ID field shared with ADAPTX
--      ,OR_SURGICAL_CASE_IDENTIFIER
